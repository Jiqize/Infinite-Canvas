...
GITHUB_REPO_URL = "https://github.com/Jiqize/Infinite-Canvas"
GITHUB_VERSION_URL = "https://raw.githubusercontent.com/Jiqize/Infinite-Canvas/main/VERSION"
GITHUB_TREE_URL = "https://api.github.com/repos/Jiqize/Infinite-Canvas/git/trees/main?recursive=1"
GITHUB_RAW_ROOT = "https://raw.githubusercontent.com/Jiqize/Infinite-Canvas/main"
...
def normalize_provider(item):
    ...
    # New Tudou entries use the generic OpenAI protocol plus the vendor's async
    # image mode. Existing stored `protocol=tudou` values remain visible and
    # round-trip unchanged until the user edits them in the advanced settings.
    if is_tudou_base_url(base_url) and image_request_mode == "openai":
...
async def export_minimax_timeline(payload: MiniMaxTimelineExportRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail="MiniMax 时间线导出超时，请缩短素材后重试") from exc
    finally:
...
async def apimart_midjourney_request(provider, path, body):
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail="Midjourney 请求超时，请稍后重试。") from exc
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail=f"Midjourney 网络请求失败：{exc}") from exc
    try:
        raw = response.json()
    except ValueError:
...
async def midjourney_result(provider, task_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail="Midjourney 任务查询超时，请稍后重试。") from exc
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail=f"Midjourney 任务查询失败：{exc}") from exc
    try:
        raw = response.json()
    except ValueError:
        raw = {"message": response.text[:500]}
