...
const API_PROTOCOLS = ['openai', 'apimart', 'gemini', 'volcengine', 'runninghub', 'jimeng', 'codex', 'gemini-cli', 'tudou'];
...
function broadcastStudioApiChange(type='providers-changed'){
    ...
    try { if(window.parent && window.parent !== window) window.parent.postMessage(message, targetOrigin); } catch(e) {}
    try { if(window.top && window.top !== window && window.top !== window.parent) window.top.postMessage(message, targetOrigin); } catch(e) {}
}
...
function renderEditor(){
    ...
    if(protocolInput){
        ...
        let legacyOption = protocolInput.querySelector('option[data-legacy-protocol="tudou"]');
        if(protocolValue === 'tudou' && !legacyOption){
            legacyOption = document.createElement('option');
            legacyOption.value = 'tudou';
            legacyOption.dataset.legacyProtocol = 'tudou';
            legacyOption.textContent = '土豆 API（兼容旧配置）';
            legacyOption.disabled = true;
            protocolInput.appendChild(legacyOption);
        } else if(protocolValue !== 'tudou' && legacyOption){
            legacyOption.remove();
        }
